	...
	void lcd_integer( int num)  // integer function
	{
	int a[10],i=0;
	if(num==0)              //if num is zero,print'0'
	{
	lcd_data('0');
	}
	if(num<0)               //if negative,print minus sign
	{
	num=-num;
	lcd_data('-');
	}
	while(num>0)    //extract individual digits
	{
	a[i]=num%10+48; //convert numeric to ascii
